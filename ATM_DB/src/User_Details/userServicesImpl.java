...
public class userServicesImpl implements userServices{
	...
	public void createAccount(UserDetails user) {
		...
		try {
			...
			System.out.println("Enter a password of 8 digit or more:");
			st1.setString(6, sc.next());
			
			
			
			st1.executeUpdate();
			ac=userServicesImpl.getAccountNumber();
	...
	public void getAccountById() {
		...
		try {
			...
			acc_n=userServicesImpl.getAccountNumber();
			...
			System.out.printf("%12s %10s %10s %10s\n",
			 ...
			 while (rs.next()) {
	                System.out.printf("%12d %10s %10s %10d.00\n",rs.getLong("Acc_number"),rs.getString("Acc_fname"),rs.getString("Acc_lname"),rs.getLong("Acc_balance"));
	            }
			}
	...
	public void updateAccountById(long acc_number) {
		...
		if(flag) {
			...
			if(ch==1) {
				...
				try {
					sql="UPDATE User_Details SET Acc_fname='"+name+"' WHERE Acc_number="+acc_number;
				...
				PreparedStatement st2;
				try {
					sql="UPDATE User_Details SET Acc_mobile='"+number+"' WHERE Acc_number="+acc_number;
					st2 = connectionToDb.con.prepareStatement(sql);
					st2.executeUpdate();
					//ResultSet rs1 = st2.executeQuery();
					System.out.println("Successfully updated");
	...
	public void deleteAccountById(long acc_number) {
		...
		//boolean flag=accountExist(acc_number);
		try {
			PreparedStatement st2=connectionToDb.con.prepareStatement("DELETE FROM User_Details WHERE Acc_number="+acc_number);
			st2.executeUpdate();
	...
	public boolean loginAccountById(long acc_number, long acc_password) {
		// TODO Auto-generated method stub
		boolean flag=accountExist(acc_number),f=false;
		long password;
		if(flag==false) {
			System.out.println("Account doesn't exist");
		...
		if(flag) {
			PreparedStatement st2;
			try {
				st2 = connectionToDb.con.prepareStatement("SELECT Acc_password FROM User_Details WHERE Acc_number="+acc_number);
				ResultSet rs1 = st2.executeQuery();
				long p=0;
				while(rs1.next()) {
				 p=rs1.getLong("Acc_password");}
				if(p==acc_password) {
					f=true;
				}
				else {
...
//		}
	...
	public static long getAccountNumber() {
		long ac=0;
		 sql1="SELECT MAX(Acc_number) FROM User_Details";
		PreparedStatement st;
		try {
			st = connectionToDb.con.prepareStatement(sql1);
			ResultSet rs = st.executeQuery();
